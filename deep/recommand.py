...
data = open(r'''.\deep\u.dat''')
user_data = data.read().split()
user_len = (int)(counts[0][0])
u = np.random.rand(user_len*feature_len)
index = 0
for i in user_data[1:]:
    u[index] = i
    index+=1
u = u.reshape((user_len,feature_len))
data.close()

data = open(r'''.\deep\p.dat''')
restaurant_data = data.read().split()
restaurant_len = (int)(counts[0][1])
p = np.random.rand(restaurant_len*feature_len)
index = 0
for i in restaurant_data[1:]:
    p[index] = i
    index+=1
p = p.reshape((feature_len,restaurant_len))
data.close()
data.close()
